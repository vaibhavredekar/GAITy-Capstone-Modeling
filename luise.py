...
def streamlit_safe_call(fn, *args, **kwargs):
    try:
        return fn(*args, **kwargs)
    except Exception as e:
        logger.error("Streamlit-safe error", exc_info=True)
        st.error("⚠️ An internal error occurred. The app is still running.")
        return None
...
# ==============================
# ROBUST FILE RESOLUTION LAYER
# ==============================

def get_project_root() -> Path:
    """
    Resolve project root robustly:
    - Works with Streamlit, CLI, Docker
    - Never throws
    """
    try:
...
def find_file_robust(
    relative_path: str,
    search_roots: Optional[List[Path]] = None,
    max_depth: int = 5
) -> Optional[Path]:
    """
    Robustly locate a file without crashing.
    ...
    try:
        roots = search_roots or [
            ...
            get_project_root(),
            Path.cwd().parent,
            Path(sys.argv[0]).resolve().parent if sys.argv else None,
        ]
        ...
        for root in roots:
            ...
            # Controlled recursive search
            for p in root.rglob(Path(relative_path).name):
                try:
                    if p.is_file():
                        return p.resolve()
                except Exception:
        ...
        logger.error(f"File resolution failed safely: {e}")
        return None
...
def init_session_state():
    ...
    defaults = {
        ...
        'imputation_values': None  # Store imputation values for missing data
    }
...
# Constants
MODEL_PATHS = {
    'baseline': "models/baseline/xgboost_model.bin",
    'baseline_ubj': "models/baseline/xgboost_model.ubj",
    'binary': "models/advance/binary_model_full.bin",
    'multi': "models/advance/multi_label_model_full.bin",
}
...
class ModelManager:
    ...
    #     """Load the baseline XGBoost model safely and align expected feature names.

    #     Key points:
    #     - Avoid pickling the XGBClassifier wrapper (fragile, can cause recursion errors).
    #     - Use XGBoost native model load every time.
    #     - Cache only a hash marker to skip redundant work.
    #     - Prefer expected feature names from the model file when available.
    #     """
    #     try:
    ...
    #         # Hash marker cache (no pickling)
    #         cache_key = f"baseline_{model_path.stem}"
    #         hash_file = MODEL_CACHE_DIR / f"{cache_key}.md5"
    #         try:
    #             with open(model_path, 'rb') as f:
    #                 current_hash = hashlib.md5(f.read()).hexdigest()
    #         except Exception as e:
    #             logger.warning(f"Could not hash baseline model file: {e}")
    #             current_hash = None

    #         # Build wrapper with the same params used in training (must match)
    ...
    #         # Load via XGBoost native loader
    ...
    #         # Prefer expected feature names from the model file (single source of truth)
    #         expected = None
    #         try:
    ...
    #         except Exception as e:
    #             logger.warning(f"Could not read feature names from booster: {e}")

    #         if expected and len(expected) > 0:
    #             FeatureEngineer.BASELINE_FEATURE_COLS = expected
    #             st.session_state.feature_columns = expected
    #             logger.info(f"Baseline expected features taken from model file: {len(expected)}")
    #         else:
    #             st.session_state.feature_columns = list(FeatureEngineer.BASELINE_FEATURE_COLS)
    #             logger.info(f"Baseline expected features taken from code list: {len(FeatureEngineer.BASELINE_FEATURE_COLS)}")

    #         # Default imputation values (0.0 unless you have medians from training)
    ...
    #             except Exception as e:
    #                 logger.warning(f"Failed to write baseline hash marker: {e}")

    #         loaded_from = "hash match" if (current_hash and hash_file.exists() and hash_file.read_text().strip() == current_hash) else "file"
    #         return model, f"Baseline model loaded successfully ({loaded_from})"
    ...
    def load_baseline_model():
        try:
            if not DEPENDENCIES.get('xgboost'):
                return None, "XGBoost unavailable"

            model_path = None
            for key in ("baseline", "baseline_ubj"):
                model_path = find_file_robust(MODEL_PATHS[key])
                if model_path:
                    break

            if not model_path:
                logger.warning("Baseline model not found — app continues without it")
                return None, "Baseline model missing"
            ...
            return model, "Baseline model ready"

        except Exception as e:
            logger.error("Baseline load failed safely", exc_info=True)
            return None, "Baseline model failed (safe fallback)"
...
class FeatureEngineer:
    ...
    def prepare_features_for_baseline(features: Dict) -> Tuple[np.ndarray, str]:
        ...
        Expects a dict mapping feature_name -> scalar.
        Produces a (1, N) float32 array in the exact expected order.
        """
        try:
            if features is None:
            ...
            if not isinstance(features, dict):
                return None, f"Baseline model requires engineered feature dict, got {type(features).__name__}"

            if len(features) == 0:
                return None, "Empty features dict"

            expected_cols = list(FeatureEngineer.BASELINE_FEATURE_COLS)

            # Diagnostics: missing / extra keys
            feature_keys = set(features.keys())
            missing = [c for c in expected_cols if c not in feature_keys]
            extra = sorted(list(feature_keys.difference(expected_cols)))

            if missing:
                logger.warning(f"Baseline feature dict missing {len(missing)} / {len(expected_cols)} features (will fill with 0.0). Example: {missing[:8]}")
            if extra:
                logger.info(f"Baseline feature dict has {len(extra)} extra keys (ignored). Example: {extra[:8]}")
            ...
            # Handle NaN values - match training behavior (here: 0.0)
            feature_array = np.nan_to_num(feature_array, nan=0.0)

            # Apply imputation values if available (optional)
            if hasattr(st.session_state, 'imputation_values') and st.session_state.imputation_values:
                for i, col in enumerate(expected_cols):
                    if feature_array[0, i] == 0.0 and col in st.session_state.imputation_values:
                        feature_array[0, i] = float(st.session_state.imputation_values[col])

            logger.info(f"Prepared baseline features array with shape {feature_array.shape}")
            return feature_array, f"Features prepared for baseline prediction (n={feature_array.shape[1]})"

        except Exception as e:
            logger.error(f"Feature preparation failed: {e}")
            logger.error(traceback.format_exc())
            return None, f"Feature preparation failed: {str(e)}"
        ...
        if uploaded_file:
            ...
            if file_path:
                ...
                try:
                    ...
                    with col4:
                        st.metric("Landmarks", df['landmark_id'].nunique() if 'landmark_id' in df.columns else 'N/A')                    # Auto-detect input type (landmarks vs precomputed features)
                    required_landmark_cols = {'frame', 'landmark_id', 'x_norm', 'y_norm', 'z_norm'}
                    cols_set = set(df.columns)

                    # Heuristics:
                    # - Landmarks CSV: has the required landmark columns
                    # - Features CSV: does NOT have required landmark columns, but has many numeric columns and overlaps with baseline feature names
                    is_landmarks = required_landmark_cols.issubset(cols_set)
                    baseline_feat_cols = list(getattr(FeatureEngineer, 'BASELINE_FEATURE_COLS', []))
                    overlap_with_baseline = len(set(baseline_feat_cols).intersection(cols_set)) if baseline_feat_cols else 0
                    is_features = (not is_landmarks) and (overlap_with_baseline > 5 or df.shape[1] >= 50)

                    detected_type = "Landmarks (raw MediaPipe)" if is_landmarks else ("Features (precomputed)" if is_features else "Unknown")
                    input_type = st.selectbox(
                        "Detected CSV type",
                        options=["Landmarks (raw MediaPipe)", "Features (precomputed)", "Unknown"],
                        index=["Landmarks (raw MediaPipe)", "Features (precomputed)", "Unknown"].index(detected_type),
                        help="Auto-detected from column names. You can override if needed."
                    )

                    if input_type == "Features (precomputed)":
                        st.success("✅ Features CSV detected — skipping landmark validation & feature extraction")
                    
                        # Preview
                        with st.expander("📊 Data Preview", expanded=False):
                            st.dataframe(df.head(20))
                    
                        # Build a single feature vector from the features CSV
                        numeric_df = df.select_dtypes(include=['number']).copy()
                        # Drop obvious label/target columns if present
                        drop_like = [c for c in numeric_df.columns if c.lower().startswith('label') or c.lower() in ('target', 'y', 'class')]
                        if drop_like:
                            numeric_df = numeric_df.drop(columns=drop_like, errors='ignore')
                    
                        if numeric_df.shape[1] == 0:
                            st.error("❌ No numeric feature columns found in this CSV.")
                            st.stop()
                    
                        agg = st.radio(
                            "How should multiple rows be combined into one feature vector?",
                            options=["Mean across rows (recommended)", "Median across rows", "Use first row"],
                            index=0,
                            help="If your features CSV contains multiple windows/segments, we combine them into one vector for prediction."
                        )
                        if agg.startswith("Mean"):
                            feature_series = numeric_df.mean(axis=0)
                        elif agg.startswith("Median"):
                            feature_series = numeric_df.median(axis=0)
                        else:
                            feature_series = numeric_df.iloc[0]
                    
                        features_dict = feature_series.to_dict()
                        st.session_state.features = features_dict
                        st.session_state.features_type = "baseline"
                        st.session_state.processing_history.append("Loaded precomputed features from CSV")
                    
                        st.info(f"Prepared a baseline feature dict with {len(features_dict)} numeric columns. The model will select/align expected features automatically.")
                    
                        # Step 3: Model Prediction (direct)
                        ...
                        if not st.session_state.available_models:
                            ...
                            model_choice = st.selectbox(
                                ...
                                key="model_choice_features_csv"
                            )
                    
                            if model_choice != "Baseline XGBoost":
                                st.warning("This CSV contains baseline-style scalar features. For ST-GCN models, please upload raw landmarks and run ST-GCN feature extraction.")
                            else:
                                if st.button("🎯 Make Baseline Prediction", key="make_prediction_features_csv", type="primary"):
                                    with st.spinner("Making prediction..."):
                                        features_array, prep_status = FeatureEngineer.prepare_features_for_baseline(features_dict)
                    ...
                    elif input_type == "Landmarks (raw MediaPipe)":
                        # Validate landmarks CSV
                        ...
                        if validation_errors:
                            ...
                            st.success("✅ CSV validation passed (landmarks detected)")
# Data preview
                        with st.expander("📊 Data Preview", expanded=False):
                            st.dataframe(df.head(20))
                                    ...
                                    if not st.session_state.available_models:
                                        ...
                                        if st.button("🎯 Make Prediction", key="make_prediction", type="primary"):
                                            with st.spinner("Making prediction..."):
                                                ...
                                                if model_choice == "Baseline XGBoost":
                                                    # Baseline always requires engineered dict of scalar features.
                                                    baseline_features = None
                                                    if st.session_state.features_type == "baseline":
                                                        baseline_features = st.session_state.features
                                                    else:
                                                        # If we extracted ST-GCN features, baseline features may be nested.
                                                        if isinstance(st.session_state.features, dict) and "baseline_features" in st.session_state.features:
                                                            baseline_features = st.session_state.features.get("baseline_features")

                                                    if baseline_features is None:
                                                        st.error("❌ Baseline prediction requires engineered baseline features. Please run baseline feature extraction first.")
                                                    else:
                                                        features_array, prep_status = FeatureEngineer.prepare_features_for_baseline(baseline_features)
                                                        if features_array is None:
                                                            st.error(f"❌ {prep_status}")
                                                        else:
                                                            prediction_result, pred_status = PredictionEngine.predict_with_baseline(
