...
class VideoConverter:
    ...
    def ensure_web_compatible(cls, video_path: Path) -> Path:
        ...
        if cls.convert_with_ffmpeg(video_path, output_path):
            return output_path
        elif cls.convert_with_opencv(video_path, output_path):
            return output_path
...
class FileManager:
    ...
    def find_output_videos(video_path: Path) -> Dict[str, Optional[Path]]:
        ...
        return {
            'annotated': next((p for p in OUTPUT_DIR.glob(f"*{stem}*annotated*")), None),
            'skeleton': next((p for p in OUTPUT_DIR.glob(f"*{stem}*skeleton*")), None),
            'csv': next((p for p in OUTPUT_DIR.glob(f"*{stem}*.csv")), None)
        }
...
class GaitAnalysisEngine:
    ...
    def find_heel_strikes(pose, foot="left", fps=30, min_time_between_steps=0.5, smooth_sigma=1):
        ...
        if foot == "left":
            heel_idx = GaitAnalysisEngine.LEFT_HEEL
        else:
            heel_idx = GaitAnalysisEngine.RIGHT_HEEL
        
        heel_y = pose[:, heel_idx, 1]
        
        if smooth_sigma and smooth_sigma > 0:
            heel_y = gaussian_filter1d(heel_y, sigma=smooth_sigma)
        
        min_distance = max(1, int(min_time_between_steps * fps))
        peaks, _ = find_peaks(-heel_y, distance=min_distance)
        return peaks
        """
        Find heel strike events from pose data
        """
        from scipy.signal import find_peaks
        from scipy.ndimage import gaussian_filter1d
                ...
                if st.button("🔍 Compare Models", use_container_width=True, type="primary"):
                    with st.spinner("Comparing models..."):
                        try:
                            ...
                            # For advanced model
                            X_stgcn = ModelManager.get_gait_cycles_for_advanced_model(
                            ...
                            if len(y_pred_baseline) > 0 and len(y_pred_advanced) > 0:
                                ...
                                with col2:
                                    ...
                                    if model_type == "Binary Classification":
                                        prob_df = pd.DataFrame({
                                            ...
                                            'Probability': pred_proba if pred_proba is not None else [0, 0]
                                        })
                                ...
                                if y_pred_proba_baseline is not None and y_pred_proba_advanced is not None:
                                    try:
                                        ...
                                        if 'label_class' in st.session_state.features_df.columns:
                                            baseline_acc = accuracy_score(
                                                st.session_state.features_df['label_class'], y_pred_baseline
                                            )
                                        else:
                                            baseline_acc = 0.0
                                        ...
                                        if 'binary' in st.session_state.selected_advanced_model.lower():
                                            advanced_acc = accuracy_score(y_pred_advanced) if len(y_pred_advanced) > 0 else 0.0
                                        else:
                                            # For multilabel classification - average accuracy
                                            advanced_acc = np.mean([
                                                accuracy_score(y_pred_advanced[:, i]) if y_pred_advanced.shape[1] > i else 0.0
                                                for i in range(min(y_pred_advanced.shape[1] if hasattr(y_pred_advanced, 'shape') else 1, 5))
                                            ])
