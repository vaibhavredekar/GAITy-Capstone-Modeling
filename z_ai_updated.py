...
#         # --- ROBUST CLASS DETECTION (FIX FOR n_classes_ ERROR) ---
#         # Do NOT use self.multiclass_model.n_classes_
#         # Instead, derive from the probability matrix shape
#         actual_n_classes = probs.shape[1]
#         expected_n_classes = len(self.multiclass_classes)
        
#         if actual_n_classes != expected_n_classes:
#             # Safe logging (no emojis)
...
#             # Adjust class list to match actual model output to prevent IndexError
...
#             raise FileNotFoundError("Multiclass model or metadata files missing.")
#         try:
#             # Force a fresh reload to ensure it's a Classifier, not a stale Booster
#             self.multiclass_model = xgb.XGBClassifier()
#             self.multiclass_model.load_model(MULTICLASS_MODEL_PATH)
...
#         # --- ROBUST FIX FOR n_classes_ ERROR ---
#         # We no longer use self.multiclass_model.n_classes_.
#         # We derive the actual number of classes from the probability matrix shape.
#         # This handles cases where loaded model is a Booster or has attribute issues.
#         actual_n_classes = probs.shape[1]
#         expected_n_classes = len(self.multiclass_classes)
        
#         if actual_n_classes != expected_n_classes:
#             # Safe logging (no emojis to prevent UnicodeEncodeError)
#             log_msg = f"Model Mismatch: Model output contains {actual_n_classes} classes, but metadata lists {expected_n_classes}. Adjusting to {actual_n_classes}."
#             ui_msg = f"⚠️ **Model Mismatch**: Model output contains {actual_n_classes} classes, but metadata lists {expected_n_classes}. Adjusting to {actual_n_classes}."
...
#             # Slice the class list to match the model's actual output capacity
#             self.multiclass_classes = self.multiclass_classes[:actual_n_classes]
...
#         # Add probability columns dynamically based on actual output
#         # We iterate up to actual_n_classes to prevent IndexError
#         for i in range(actual_n_classes):
#             cls_name = self.multiclass_classes[i] if i < len(self.multiclass_classes) else f"Class_{i}"
#             details_df[f'prob_{cls_name}'] = probs[:, i]
...
#             raise FileNotFoundError("Binary model or metadata files missing.")
#         try:
#             # Force a fresh load (clears any stale cache/wrapper issues)
#             self.binary_model = xgb.XGBClassifier()
#             self.binary_model.load_model(BINARY_MODEL_PATH)
#             with open(BINARY_FEATURES_PATH, 'r') as f: 
#                 self.binary_features = json.load(f)
...
#             raise FileNotFoundError("Multiclass model or metadata files missing.")
#         try:
#             # Force a fresh load to ensure internal integrity, avoiding wrapper attribute errors
#             self.multiclass_model = xgb.XGBClassifier()
#             self.multiclass_model.load_model(MULTICLASS_MODEL_PATH)
#             with open(MULTICLASS_METADATA_PATH, 'r') as f: 
#                 self.multiclass_metadata = json.load(f)
...
#         # --- ROBUST FIX FOR n_classes_ ERROR (XGBOOST.SKLEARN SPECIFIC) ---
#         # The previous fix used self.multiclass_model.n_classes_, which fails with scikit-learn wrappers.
#         # We MUST derive n_classes from the probability matrix shape, as the wrapper doesn't support it correctly.
#         # probs shape is (N_samples, N_classes). shape[1] is N_classes.
#         actual_n_classes = probs.shape[1]
#         # ----------------------------------------------------------------
...
#         if actual_n_classes != expected_n_classes:
#             # Safe logging (no emojis to avoid UnicodeEncodeError on Windows)
#             log_msg = f"Model Mismatch: Model output contains {actual_n_classes} classes, but metadata lists {expected_n_classes}. Adjusting to {actual_n_classes}."
#             ui_msg = f"⚠️ **Model Mismatch**: Model output contains {actual_n_classes} classes, but metadata lists {expected_n_classes}. Adjusting to {actual_n_classes}."
...
#             # Slice the class list to match the actual model output to prevent IndexError in the loop below
#             self.multiclass_classes = self.multiclass_classes[:actual_n_classes]
...
#         # Map predictions safely
#         # We must iterate up to actual_n_classes, not len(self.multiclass_classes)
#         # Because self.multiclass_classes has been sliced above to match reality.
#         for i, p in enumerate(preds):
#             # Only map if the prediction index is valid for the actual number of classes
#             if p < actual_n_classes:
#                 labels.append(self.multiclass_classes[p])
#                 confidences.append(probs[i, p])
#             else:
#                 # Fallback for indices that are somehow out of bounds (shouldn't happen with predict output, but robust check here)
#                 labels.append("Unknown")
#                 confidences.append(0.0)
...
#         # Add probability columns dynamically based on actual model output
#         # We iterate up to actual_n_classes.
#         for i in range(actual_n_classes):
#             cls_name = self.multiclass_classes[i] if i < len(self.multiclass_classes) else f"Class_{i}"
#             details_df[f'prob_{cls_name}'] = probs[:, i]
...
def main():
        ...
        if model_df is None:
            ...
            if st.session_state.pred_multiclass_results is not None:
                ...
                with st.expander("View Detailed Probability Breakdown"):
                    st.dataframe(res_df[display_cols])
        # ════════════════════════════════════════════════════════════════════════════════════════
    # TAB 6: AI MODELLING (FIXED UNICODE LOGGING & n_classes_ & BALLOON ANIMATION)
    # ══════════════════════════════════════════════════════════════════════════════════════
    ...
    #                 with st.spinner("Loading Model..."):
    #                     try:
    #                         # Reload model to ensure fresh state (clears wrapper cache)
    #                         st.session_state.predictor.load_binary_model()
    ...
    #                         with st.spinner("Balancing Features..."):
    #                             # Handle the version mismatch logic
    #                             pass # Logic handled inside predict
...
# ----------- Working for multi-class predictions ------------------#
...
# # ═════════════════════════════════════════════════════════════════════════════════
# # MODEL PREDICTOR (UPDATED TO FIX MULTI-CLASS ERROR)
# # ═════════════════════════════════════════════════════════════════════════════════
...
#         if actual_n_classes != expected_n_classes:
#             warning_msg = f"⚠️ **Model Mismatch**: Model output contains {actual_n_classes} classes, but metadata lists {expected_n_classes}. Adjusting to {actual_n_classes}."
#             logger.warning(warning_msg)
#             st.warning(warning_msg)
            
#             # Slice the class list to match the model's actual output
#             # This prevents "IndexError: index X is out of bounds"
#             self.multiclass_classes = self.multiclass_classes[:actual_n_classes]
...
#                 confidences.append(probs[i, p])
#             else:
#                 # Fallback for out-of-bounds indices (shouldn't happen with XGBoost predict)
#                 labels.append("Unknown")
#                 confidences.append(0.0)
...
#     # ═════════════════════════════════════════════════════════════════════════════════
#     # TAB 6: AI MODELLING (FIXED MULTI-CLASS HANDLING)
#     # ═════════════════════════════════════════════════════════════════════════════════
...
# Working binary predictions below##
...
# # Model Paths
# BINARY_MODEL_PATH = MODELS_DIR / "xgboost_model.bin"
# BINARY_FEATURES_PATH = MODELS_DIR / "feature_names.json"  # Meta for Binary
# MULTICLASS_MODEL_PATH = MODELS_DIR / "xgboost_gait_5class.bin"
# MULTICLASS_METADATA_PATH = MODELS_DIR / "xgboost_gait_5class_metadata.json"  # Meta for Multi
...
#         try:
#             # Assuming the external script follows a specific interface
#             config = mp_module.PipelineConfig.from_json(CONFIG_PATH)
#             pipeline = mp_module.PoseDetectionPipeline(config)
#             results = pipeline.run()
#             logger.info(f"Pipeline complete: {len(results)} results")
...
#         for i, cls_name in enumerate(self.multiclass_classes):
#             details_df[f'prob_{cls_name}'] = probs[:, i]
#         return details_df
...
#             # Multiclass Prediction
#             st.markdown("### Multiclass Prediction (5 Specific Classes)")
#             if m_model_ok and m_meta_ok:
#                 if st.button("▶️ Run Multiclass Prediction", key="run_multi"):
#                     with st.spinner("Loading Model..."):
#                         try:
...
#                 # Aggregate stats
#                 counts = res_df['prediction'].value_counts()
#                 st.write("**Prediction Distribution (5 Classes):**")
#                 st.bar_chart(counts)
